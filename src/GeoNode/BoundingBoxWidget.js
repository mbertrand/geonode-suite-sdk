...
GeoNode.BoundingBoxWidget = Ext.extend(Ext.util.Observable, {
    ...
    applyConstraint: function(query) {
        ...
        if (this.hasConstraint()) {
            ...
            query.bbox = [bounds.left,bounds.right,bounds.bottom,bounds.top].toString();
        }
        else if (this._ready) {
    ...
    initFromQuery: function(grid, query) {
        if (query.bbox) {
            var bounds = new OpenLayers.Bounds.fromString(query.bbox);
			query.bbox = [bounds.left,bounds.right,bounds.bottom,bounds.top].toString(0);
            if (bounds) {
                bounds.transform(
                    new OpenLayers.Projection("EPSG:4326"),
                    new OpenLayers.Projection(this.viewerConfig.map.projection)
